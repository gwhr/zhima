  ...
  try {
    ...
    const [platformConfig, recharged] = await Promise.all([
      ...
      hasUserRecharged(session!.user.id).catch((err) => {
        console.warn("hasUserRecharged failed in workspace detail GET, fallback=false", err);
        return false;
      }),
    ]);
    ...
    console.error("Workspace detail failed:", err);
    if (
